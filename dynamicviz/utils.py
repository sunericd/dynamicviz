...
def convert_anndata (adata, obs, obsm="X_pca", n_components=None):
    ...
    Arguments:
        ...
        use = str, str correspond to a key in adata.obsm (i.e. "X_pca") -- if no matches found, will default to adata.X
...
def regenerate_anndata (adata, df, bootstrap_number, obsm):
    ...
    adata_copy.obsm[obsm] = df[df["bootstrap_number"] == bootstrap_number][["x1", "x2"]].values # update UMAP
